...
import ComponentSetting from '../SettingTeacher/componentSetting';
...
const SettingNotification = (props) => {
  ...
  const didMount = useCallback(async () => {
    try {
      ...
      if (deviceId) {
        await dispatch(loadSettingApp(deviceId, dataLogin.jwt_token));
      } else {
        alertError();
      }
    } catch (error) {
          ...
          <>
            <View style={styles.container}>
              <View style={styles.viewTop}>
                ...
                <ComponentSetting
                  title="Khi có học sinh xin tham gia vào lớp"
                ...
                />
                <ComponentSetting
                  title="Khi có học sinh mới được thêm vào lớp"
                  image={'student_setting_image8'}
                  status={studentApproved?.value}
                  checkTrue={checkStudentApproved}
                  type={typeBtn.switch}
                />
                <ComponentSetting
                  title="Khi học sinh đã hoàn thành hết bài tập được giao trong lớp"
                  image={'student_setting_image8'}
