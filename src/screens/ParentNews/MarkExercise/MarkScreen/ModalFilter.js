...
const ModalFilter = props => {
  ...
  const {_showModalFilter, _applyFilter, setIsPressFilter, isPressFilter} =
    props;
  ...
  const checkIsFilter = () => {
    ...
    dataLevel.forEach(element => {
      if (element.choose != levelCheck) isAll = false;
    });
    var skillCheck = dataSkill[0].choose;
    dataSkill.forEach(element => {
      if (element.choose != skillCheck) isAll = false;
    });
        ...
        <View style={styles.containerContent}>
          <View style={styles.txtButtonHead}>
            ...
            <TouchableOpacity onPress={cancelModal} style={styles.btnCancel}>
              <TextBox style={styles.txtCancel}>
                {language.CourseFilterModal.CloseBt}
              </TextBox>
            </TouchableOpacity>
          </View>
