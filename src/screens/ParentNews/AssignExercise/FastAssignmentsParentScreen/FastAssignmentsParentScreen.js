...
import {FastAssignmentsMethod} from './FastAssignmentsParentScreen.logic';
import {styles} from './FastAssignmentsParentScreen.style';
import {ShortMainButton} from '../../../../componentBase/ShortMainButton';
import {SmallCheckBox} from '../../../../componentBase/SmallCheckBox';
import {ParentText} from '../../../../stringJSON/ParentTextJson';

/**
 * FastAssignmentsParentScreen Screen - Giao bài - Chọn học viên (luồng giao bài full)
 * @param {object} props props from redux and navigation
 * @returns {Component}
 */
export const FastAssignmentsParentScreen = props => {
  let {
    onChooseStudent,
    onChooseAll,
    startDate,
  ...
  const checkChosenAllStudent = () => {
    let filterLength = listStudent.filter(
      item => item.status_assign === 1,
    ).length;
    ...
    return (
      <View style={styles.zero}>
        <Text style={styles.zeroText}>
          {'Lớp chưa có Học sinh, hãy thêm học sinh vào lớp để giao bài.'}
        </Text>
      </View>
    );
  };
  ...
  return (
    <>
      <AppHeader
        ...
        title={
          role !== 'SettingAssign'
            ? language.SettingAssignmentsScreen.EditHeader
            : language.SettingAssignmentsScreen.Header
        }
        leftIconOnPress={() => {
        ...
        <View style={styles.body}>
          <KeyboardAwareScrollView
            ...
            <View style={styles.viewStudent}>
              ...
              {/* <View style={styles.row}>
                  ...
                  <View style={styles.marginLeft20}>
                    <SmallCheckBox
                      disabled={
                        role !== 'SettingAssign' && checkChosenAllStudent()
                      }
                      onPress={onChooseAll}
                      isNotify={chooseAll}
                      size={SmartScreenBase.smBaseWidth * 50}
            ...
            {remind && (
              <View style={styles.marginBottom20}>
                ...
                <View style={[styles.row, styles.marginTop10]}>
                  <TextBox style={styles.txtRemind}>
                    {language.SettingAssignmentsScreen.RemindText_1}
                  </TextBox>
                  <View
                    style={[
                      styles.ipNumberDate,
                      !!err ? styles.errBorder : styles.shadow,
                    ]}>
                    <TextInput
                      onChangeText={value => {
            ...
            <ShortMainButton
              ...
              text={
                role === 'SettingAssign'
                  ? language.SettingAssignmentsScreen.MainButton_1
                  : language.SettingAssignmentsScreen.MainButton_2
              }
