...
import {
    ...
    TouchableOpacity, Alert, Keyboard, FlatList
} from "react-native";
...
const AddInfoScreen = (props) => {
    ...
    const {email, password, rePassword} = params;
    ...
    const _onSubmit = async () => {
        const role = dataRole.find(item => item.choose).value;
        if (!fullName || (role === 'student' && !grade && !school) || (role === 'teacher' && !school)) {
            ...
            const data = qs.stringify({
                email, password, re_password: rePassword, fullname: fullName, gender, role, class: grade, school,
            });
            try {
                const res = await axios({method: 'post', url, headers, data});
                Alert.alert('Thông báo', res.data.msg,
                    [{text: "Đồng ý", onPress: () => _goToLogin(res.data.status)}],
                    {cancelable: false}
                );
                setLoading(false);
            } catch (e) {
                console.log('add info', e);
    ...
    const _goToLogin = (status) => {
      if(status){
          props.navigation.navigate('LoginScreen', {username: email, password})
      }
    };
                ...
                {loading ? <LoadingScreen/> :
                    <View style={{flex: 1, alignItems: 'center', justifyContent: 'flex-end'}}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={_onSubmit}
                        >
                            <Text style={{fontSize: SmartScreenBase.smPercenWidth * 4.5, color: '#fff'}}>Tạo tài
                                khoản</Text>
                        </TouchableOpacity>
