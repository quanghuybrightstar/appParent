...
import { FlatList, RefreshControl, ImageBackground, Platform, ScrollView, Text, TouchableOpacity, View, Image } from 'react-native'
import { useSelector } from 'react-redux'
import { styles } from "./AttendanceCardTeacherScreen.styles";
import { useModel } from './AttendanceCardTeacherScreen.logic'
...
/**
 * AttendanceCardTeacherScreen Screen
 * @param {object} props props from redux and navigation
 * @returns {Component}
 */
export const AttendanceCardTeacherScreen = (props) => {
    const language = useSelector(state => state.LanguageStackReducer.language)
    const { listStudent, loading, baseUrl, status, modalTitle, isProcess, attendanceDetail, errorMessage,
        selectedStudent, setSelectedStudent, isCommentVisible, onCancelModal, focusStudent, onSave, isHasPrivate, setSavePri, isSavePri,
        isSelectAll, onSelectItem, onSelectAll, onToggleCommentModal, generalComment, onSubmitComment, onStatusChange,
        isShowSendOption, setShowSendOption, selectedSendOption, setSelectedSendOption, onPreSave, setshowModalSelect, showModalSelect , onShowSelect, curSelect } = useModel(props)
    const { AttendancePaperTeacherScreen } = language
    ...
    return (
        <View style={[styles.container, styles.background]}>
            <AppHeader title={moment(attendanceDetail.date_roll_up, 'DD-MM-YYYY').format('DD/MM/YYYY')} leftIconOnPress={() => {
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => (
                            <View style={[styles.itemBox, styles.boxShadow,]}>
                                <View>
                                    <RoundAvatar avatar={item.avatar ? `${baseUrl}${item.avatar}` : null} gender={item.gender ? item.gender : null} width={SmartScreenBase.smBaseHeight * 100} height={SmartScreenBase.smBaseHeight * 100} />
                                </View>
                                <View style={styles.infoContainer}>
                                    <View style={[styles.infoBox, styles.nameBox]}>
                                        <TextBox text={item.fullname} style={styles.nameText} />

                                        {/* {!!item.send_to_parent && item.send_to_parent === 1 ? < View style={[styles.row]}>
                                    ...
                                    <View style={styles.infoBox}>
                                        <View style={styles.container}>
                                            {/* <ShortMainButton textNormal={true} onPress={() => {
                                                ...
                                                text={AttendancePaperTeacherScreen.FixCommentBt}
                                                widthType={'mini'}
                                            /> */}
            ...
            >
                <View style={styles.modal}>
                    ...
                    <View style={styles.attendanceOptionBox}>
                        {dataSend.map(item => {
                            return (
                                <TouchableOpacity onPress={() => setSelectedSendOption(item.value)} style={[styles.rowSelection, styles.marginBotton]}>
