...
export const FastAssignmentsMethod = (props) => {
    ...
    const getStudent = () => {
        setLoading(true)
        APIBase.postDataJson('get', API.baseurl + API.GetClassMember + '?class_id=' + props.navigation.getParam('classId')).then(r => {
            console.log('studen', r.data.data)
    ...
    const onSave = () => {
        ...
        if (remind && !!numberDate && (numberDate * 3600 * 24) > ((moment(moment(endDate).format('YYYY-MM-DD')).format('X') - moment(moment(startDate).format('YYYY-MM-DD')).format('X')))) {
            setErr(EditAssignJson.errMutil)
        } else {
            if (role === 'SettingAssign') {
    ...
    const checkColorBtn = () => {
        if (remind) {
            if (!!note && !!numberDate && chooseStudent.length > 0 && err === '' && errorDate === '') {
    ...
    */
    ...
    return {
        ...
        numberDate, setnumberDate, onSave, loading, checkColorBtn, checkDisableBtn, role,
        checknumberDate, errorDate
    }
}