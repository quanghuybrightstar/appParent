...
import {ActionWrittingD2} from '../../../../redux/actions/ActionWrittingD2';
...
class WrittingD2 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
    ...
    componentDidMount() {
        ...
        for (let i = 0; i < response.data.data_question.length; i += 1) {
            ...
            for (
                ...
                DataObject2.question_id = response.data.data_question[i].question_id;
                ...
                console.log("=====da",response.data.data_question[i].list_option[j].match_option_text)
                DataObject2.explain = ""
                this.setState({
                    title_ENG:
            ...
            this.dataHint.push(response.data.data_question[i].list_option[0].match_option_text[0]);
    ...
    _RenderQuestion = ({ item, index }) => {
        ...
        console.log("=====_RenderQuestion",arr[0])
        return (
            ...
            if(e.status){
                ...
                for(let j =0; j < hints.length ; j++){
                    e.rights.push({
                        ...
                        rootText: rootList[hints[j].index]
                    })
                }
            }
        })
                    ...
                    style={{flexDirection:'row',flexWrap:'wrap',marginBottom:SmartScreenBase.smPercenWidth*3}}>
                            {
                                ...
                                key={i}>
                                <Text style={{ color: (c.show || this.state.showFullHint) ? Colors.Black : Colors.Transparent, fontSize: SmartScreenBase.smFontSize * 40 }}>{c.rootText}</Text>
                            </TouchableOpacity>)
                        }
                    </View>
                }
        ...
        return (
            <View
                ...
                <View style={{flexDirection: 'row'}}>
                    ...
                    />
                    <View style={{width: width * 0.7, marginBottom: SmartScreenBase.smPercenWidth*2}}>
                        <View
                            style={{
                                height: SmartScreenBase.smBaseWidth * 112,
                                justifyContent: 'center',
                            ...
                            <Text style={{
                                color: myAns.status?'#72B228':'#D80B0B',...StyleLesson.answer_text,
                            }}>
                                {`${myAns.ans.replace(/\n/g, ' ')}`}
                            </Text>
                        </View>
...
function mapStateToProps(state) {
    return {
        data_answer: state.WrittingD2Reducer.data_answer,
        data_login: state.AuthStackReducer.dataLogin,
    };
}

export default connect(mapStateToProps)(WrittingD2);
