...
import ItemReadingD8 from "./Component/ItemReadingD8";
...
import { ReadingD8Aciton } from '../../../../redux/actions/readingD8Action';
...
class ReadingD8 extends Component {
    ...
    componentDidMount() {
        if (this.state.testing == "aftertest") {
            const { readingD8 } = this.props;
            this.setState({ data: readingD8.data, NumberTrue: readingD8.NumberTrue, checkResuilt: false, loadding: false });
            this.ListAnswer = readingD8.ListAnswer;
        } else {
            this._getData();
        }
    ...
    convertData = (data) => {
        let dataConvert = [];
        let jamming_answer = data[0].jamming_answer.split(',');
        let option = [];
        try{
        ...
        for (let i = 0; i < data.length; i++) {
            option.push(data[i].match_option_text[0])
        }
    ...
    _OnpressAnswer(answer, index) {
        ...
        if (check.length == 0) {
            this.setState({ refresh: !this.state.refresh, disabled: false, data: array })
        }
    };
    ...
    async _OnPressCheckResuilt() {
        const { checkResuilt, data, testing } = this.state
        ...
        if (this.state.checkResuilt == null) {
            ...
            if (this.NumberCheck < 2) {
                ...
                this.setState({ checkResuilt: null, disabled: true, Choose: 0,data:array});
            } else {
                this.props.hideTypeExercise();
                if (testing == 'homeword') {
                    this._postDataAnswer()
                } else {
                    this.props.methodScreen(8);
                }
            }
        } else {
            this.props.hideTypeExercise();
    ...
    _endgame = async (data) => {
        await this.props.dispatch(ReadingD8Aciton(data));
    }
    ...
    _ShowQuestion() {
        return (
            <View>
                ...
                <View style={{ marginTop: SmartScreenBase.smPercenHeight * 2, flexDirection: "row", alignSelf: "center", alignItems: "center", justifyContent: "center" }}>
        ...
        return (
            <View style={{ alignItems: "center", alignSelf: "center" }}>
                {this.NumberCheck >= 2 || this.state.checkResuilt == true ? (
                    <View style={{
                        height: SmartScreenBase.smPercenWidth * 24,
                        width: "100%",
                        ...
                        <View style={{ height: SmartScreenBase.smBaseWidth * 200 }} />
                    )}
                <Text style={{
                ...
                <View style={{ width: SmartScreenBase.smPercenWidth * 100, alignItems: "center" }}>
                    <View style={{ height: this.NumberCheck < 2 || this.state.checkResuilt == true? SmartScreenBase.smPercenHeight * 60 : SmartScreenBase.smPercenHeight * 53 }}>
                        <FlatList
                            data={this.state.data}
    ...
    RenderItemResuilt = ({ item, index }) => {
        return (
            <View style={[StyleLesson.Sty_View_Border, {
                ...
                marginBottom: index == this.state.data.length - 1 ? SmartScreenBase.smPercenHeight * 2 : 0,
                borderColor: this.ListAnswer[index].value !== this.state.data[index].answer ? "#D80B0B" : "#388C02",
                marginTop: SmartScreenBase.smBaseWidth * 100,
                ...
                <View style={{ flexDirection: 'row' }}>
                    ...
                    <Text style={[stylesApp.txt, { paddingRight: 30, fontSize: 17 }]}>
                                        ...
                                        return (
                                            ...
                                            <View style={{ flexDirection: 'row', marginTop: 15, alignItems: 'center' }}>
                                                <Image source={{ uri: "lesson_grammar_image3", marginTop: SmartScreenBase.smPercenHeight }}
                                                    style={[StyleLesson.Image_Explain, { marginLeft: -SmartScreenBase.smPercenWidth }]}
                                                />
                                                <Text style={[stylesApp.txt, {
                                                    // textTransform: 'capitalize',
                                                    color: "#388C02",
                                                ...
                                                }]}>{this.option[number].toUpperCase()}. {string}</Text>
                                            </View>
                                        )
                                    }
                                })
                ...
                <View style={{ position: "absolute", alignSelf: "center", top: -SmartScreenBase.smBaseWidth * 120 }}>
                    <Image
...
function mapStateToProps(state) {
    return {
        readingD8: state.reading8Reducer.reading,
        dataLogin: state.AuthStackReducer.dataLogin,
        checkType: state.TesttingReducer.testing
    }
}
export default connect(mapStateToProps)(ReadingD8);
